...
def load_model(model_path):
    """加载训练好的模型"""
    model = CustomMSTPlusPlus(in_channels=3, out_channels=9, n_feat=31, stage=3)                                         # out_channels=9
...
def process_image(model, input_dir, output_dir, device):
    """处理单个场景的图像（输入通道固定为1、4、7；输出通道固定为1、4、7、8、9、10、11、12、13）"""
    # 获取输入文件（3通道：1、4、7）
    required_input_idx = [1, 4, 7]
    input_files = [os.path.join(input_dir, f"{i}.bmp") for i in required_input_idx]
    
    # 校验存在性
    if not all(os.path.exists(p) for p in input_files):
        print(f"警告: 目录 {input_dir} 中缺少必要的输入通道文件，期望: {required_input_idx}")
        return
    ...
    # 将输出转换回NumPy数组
    output_data = output_tensor.squeeze(0).cpu().numpy()  # 形状: [9, H, W]
    
    # 确保输出目录存在
    os.makedirs(output_dir, exist_ok=True)
    
    # 保存每个输出通道为单独的BMP文件（命名为1、4、7、8、9、10、11、12、13）
    required_output_idx = [1, 4, 7, 8, 9, 10, 11, 12, 13]                                                                #required_output_idx = [1, 4, 7, 8, 9, 10, 11, 12, 13]
    for i, channel in enumerate(output_data):
        channel_scaled = (channel * 255.0).clip(0, 255).astype(np.uint8)
        output_name = required_output_idx[i]
        output_path = os.path.join(output_dir, f"{output_name}.bmp")
        cv2.imwrite(output_path, channel_scaled)
        print(f"已保存通道 {output_name} 到 {output_path}")

def main():
    parser = argparse.ArgumentParser(description="使用MST++模型进行三通道到九通道的推理")
    parser.add_argument("--model_dir", type=str, default="./exp/custom_mst_plus_plus/2025_09_02_13_06_25/",
                        help="包含模型检查点的目录")
